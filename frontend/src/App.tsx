  ...
  return (
    <div className="home-content">
      ...
      {config.show_search || config.show_groups ? <div className="toolbar">
        ...
        {config.show_groups ? <div className="group-tabs" aria-label={ui(locale, "节点分组", "Server groups")}>{groups.map((item) => <button className={group === item ? "active" : ""} key={item} onClick={() => setGroup(item)}>{item === "__all__" ? ui(locale, "全部", "All") : item}</button>)}</div> : null}
      ...
      <header className="site-header">
        <div className="container header-inner">
          ...
          <div className="header-actions">
            ...
            <button className="icon-btn" onClick={() => window.location.assign("/admin")} title={ui(locale, "进入后台", "Administration")}><UserCircle size={18} /></button>
          </div>
        </div>
      </header>
