...
实时上/下行速率由 Agent 按“收发字节增量 / 实际采样秒数”计算，默认每 3 秒采集并上传一次，可设置为 3–60 秒。前台每秒合并展示已收到的最新样本，无新样本时保持原值。历史保存间隔独立，默认 60 秒。

Agent 和前台的监控数据使用 gzip 压缩的二进制 WebSocket 消息，机器静态信息仅在连接建立或发生变化时上传。正常连接只发送新增样本，后端暂存后按历史保存间隔批量写入数据库并确认；Agent 保留未确认样本，断线后补传。页面访问不会改变节点上报频率。
