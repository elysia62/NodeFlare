...
更新：同一数据库基线内重新运行安装脚本即可，已安装的配置和数据会保留；也可直接执行 `install.sh --install`（Windows 为 `install.ps1 -Install`）。从旧迁移 `01–04` 切换到本版前，必须先按[数据库基线切换](#数据库基线切换)导出备份并准备全新数据库。卸载默认保留配置和数据，追加 `--purge` 一并删除：
...
本版按新项目维护，SQLite / PostgreSQL 均只有一个包含完整结构的 `01_initial.sql`。Agent 协议为 `2`，不再接受旧协议或缺少 `persist` 标志的上报，服务端与 Agent 必须配套更新。

从已经执行旧迁移 `01–04` 的版本切换时：

1. **更新前**在原版本后台导出 ZIP 备份，并保留原程序、配置和数据库以便回退。支持该版本导出的完整表结构备份（格式版本 `2`），不支持更早缺少历史聚合字段的备份。
2. 停止服务端，将 `database_url` 指向全新的 SQLite 文件或空 PostgreSQL 数据库。不要覆盖原库、清空迁移记录，或把旧 `.db` / SQL 转储直接作为新库使用。
3. 安装本版，用临时管理员账号初始化新库；原配置中的管理员密码已被清空时，需要重新设置 `admin_username`、`admin_password`。登录新后台后恢复刚导出的 ZIP。
4. 恢复会覆盖临时账号和新库业务数据，并退出登录。使用备份中的管理员账号、密码和两步验证重新登录，再更新各节点 Agent；使用额外安装 Token 的节点还需重新安装以更换 Token。

ZIP 恢复只导入业务数据，不导入 `_sqlx_migrations`，新库仍保留单一 `01` 基线。现版本备份中未填充的历史统计字段会在恢复事务内补齐；设置、节点、历史记录、主题文件和安全配置会保留。全部验证正常前不要删除原库。
