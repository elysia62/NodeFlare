...
- 升级建议先更新服务端再更新 Agent；支持当前版本和 v1.0.3 导出的 ZIP 备份。

SQLite / PostgreSQL 均使用一份 `01_initial.sql`，首次启动时自动创建完整数据库结构。ZIP 恢复校验表结构并导入业务数据，保留目标库的初始化记录。服务端与 Agent 须配套使用，当前 Agent 协议为 `2`。

从 v1.0.3 导入数据：

1. 在 v1.0.3 后台导出 ZIP 备份，并保留原配置和数据库。
2. 为新版配置全新的 SQLite 文件或空 PostgreSQL 数据库，设置临时管理员账号和密码后启动。
3. 登录新版后台恢复 ZIP；恢复后使用备份中的管理员账号、密码和两步验证重新登录，再更新各节点 Agent。

导入会保留设置、节点、历史数据、主题文件和安全配置；旧版最新状态中的重复指标列会忽略，完整指标从 `latest_json` 保留。原库的迁移记录不会导入。额外安装 Token 的恢复规则见上文。
